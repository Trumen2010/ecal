...
TEST(Core, CheckIfIsInit)
{
  // check if initialized
  EXPECT_EQ(0, C::eCAL_IsInitialized(0));
  // initialize eCAL API
  C::eCAL_Initialize(0, nullptr, "check if is initialized", 0);
  // check if initialized
  EXPECT_EQ(1, C::eCAL_IsInitialized(0));
}
...
TEST(Core, MultipleInitializeFinalize)
{
  ...
  {
    // initialize eCAL API
    EXPECT_EQ(0, C::eCAL_Initialize(0, nullptr, "multiple initialize/finalize", 0));

    // finalize eCAL API
    EXPECT_EQ(0, C::eCAL_Finalize(0));
  }
}

// Not working, C equivalent missed
TEST(Core, LeakedPubSub)
{
  // initialize eCAL API
  EXPECT_EQ(0, C::eCAL_Initialize(0, nullptr, "leaked pub/sub"));
